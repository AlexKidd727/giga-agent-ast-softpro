...
FEW_SHOTS_ORIGINAL = [
    HumanMessage(
        content="""<task>Покажи мой портфель акций</task> Активно планируй и следуй своему плану! Действуй по простым шагам!
Следующий шаг:"""
    ),
    AIMessage(
        content="""<thinking>
Пользователь просит показать портфель акций. Это задача для tinkoff_agent, который специализируется на работе с Tinkoff Invest API.
</thinking>

План:
1. Вызвать tinkoff_agent для получения информации о портфеле

Начинаю выполнение плана.""",
        tool_calls=[
            dict(
                name="tinkoff_agent",
                args={"user_request": "покажи мой портфель акций", "user_id": "default_user"},
                id="tinkoff_123",
            )
        ],
    ),
    ToolMessage(
        tool_call_id="tinkoff_123",
        content=json.dumps(
            {
                "status": "success",
                "message": "Портфель успешно получен",
                "data": {
                    "total_value": 150000,
                    "positions": [
                        {"symbol": "SBER", "quantity": 100, "price": 250.50},
                        {"symbol": "GAZP", "quantity": 50, "price": 180.25}
                    ]
                }
            },
            ensure_ascii=False,
    ...
    AIMessage(
        content="""Вот информация о вашем портфеле:

**Общая стоимость портфеля:** 150,000 ₽

**Позиции:**
- SBER: 100 акций по 250.50 ₽
- GAZP: 50 акций по 180.25 ₽

Портфель успешно загружен из Tinkoff Invest."""
    ),
    HumanMessage(
