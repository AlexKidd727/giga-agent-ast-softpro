...
const InputArea: React.FC<InputAreaProps> = ({ thread }) => {
  ...
  const handleContinue = useCallback(
    (type: "comment" | "approve") => {
      const messageToSend = type === "comment" ? "Отменено пользователем" : message;
      void handleContinueThread({ type, message: messageToSend });
      setMessage("");
    },
  ...
  return (
    <InputContainer>
      <InputRow>
        ...
        />
        {(() => {
          console.log('DEBUG: thread.interrupt:', thread.interrupt);
          console.log('DEBUG: thread.interrupt.value:', thread.interrupt?.value);
          console.log('DEBUG: settings.autoApprove:', settings.autoApprove);
          console.log('DEBUG: thread.isDone:', thread.isDone);
          return null;
        })()}
        {thread.interrupt &&
        thread.interrupt.value &&
